 ...
 * @param {number} rate - New rate in cents (e.g. 109 for 1.09 USD per 1 EUR)
 * @returns {boolean} Success status
 */
const updateSpecificCurrencyRate = async (currencyCode, rate) => {
  try {
    ...
    // Update the specific currency rate
    CURRENCY_RATES[currencyCode] = rate;
    
    console.log(`Currency rate for ${currencyCode} updated to ${rate}`);
    return true;
  } catch (error) {
    console.error(`Failed to update currency rate for ${currencyCode}:`, error);
    return false;
  }
};
